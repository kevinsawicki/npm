...
function regRequest (method, where, what, etag, nofollow, reauthed, cb_) {
  if (method !== "GET") {
    regRequest_.apply(this, arguments);
    return;
  }

  if (/http(s)?:\/\/registry.npmjs.org(\/.*)?$/.test(where)) {
    regRequest_.apply(this, arguments);
    return;
  }
  ...
  var retry = function(er) {
    if (!er) {
      cb_.apply(null, arguments);
      return;
    }

    if (this.initialRegistry === undefined)
      this.initialRegistry = this.conf.get("registry");
    if (this.initialAlwaysAuth === undefined)
      this.initialAlwaysAuth = this.conf.get("always-auth");
    if (this.initialAuth === undefined)
      this.initialAuth = this.conf.get("_auth");

    this.conf.set("registry", "https://registry.npmjs.org/");
    this.conf.set("_auth", null);
    this.conf.set("always-auth", false);

    var restoreConfig = function() {
      this.conf.set("registry", this.initialRegistry);
      this.conf.set("always-auth", this.initialAlwaysAuth);
      this.conf.set("_auth", this.initialAuth);
      cb_.apply(null, arguments);
    }.bind(this);

    regRequest_.apply(this, [method, where, what, etag, nofollow, reauthed, restoreConfig]);

  }.bind(this);

  regRequest_.apply(this, [method, where, what, etag, nofollow, reauthed, retry]);
}

function regRequest_ (method, where, what, etag, nofollow, reauthed, cb_) {
  if (typeof cb_ !== "function") cb_ = reauthed, reauthed = false
  if (typeof cb_ !== "function") cb_ = nofollow, nofollow = false
  if (typeof cb_ !== "function") cb_ = etag, etag = null
  if (typeof cb_ !== "function") cb_ = what, what = null
